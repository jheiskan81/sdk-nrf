...
static int write_assist(uint16_t obj_inst_id,
	...
	err = location_agps_data_process(assist_buf, sizeof(assist_buf));
	if (err) {
...
{
	LOG_INF("Writing assistance data");
	LOG_INF("Data length %u, total size %u", data_len, total_size);
	int err = 0;
	static uint32_t bytes_downloaded;

	memcpy(&assist_buf[bytes_downloaded], &data[0], data_len);
	LOG_INF("First byte %u", assist_buf[0]);
	LOG_INF("First byte of data %u", data[0]);

	bytes_downloaded += data_len;

	LOG_INF("Bytes downloaded %u", bytes_downloaded);

	if (last_block) {
		LOG_INF("Last block achieved size of buf %d", sizeof(assist_buf));
		err = location_agps_data_process(assist_buf, bytes_downloaded);
		if (err) {
			LOG_WRN("Unable to process A-GPS data, error: %d", err);
		} else {
			LOG_INF("A-GPS data processed");
